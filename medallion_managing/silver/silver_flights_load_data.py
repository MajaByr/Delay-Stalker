...
df_airports = (
    df_airports_csv.withColumnRenamed("city", "city_name")
    .join(
...
df_routes = df_routes.dropDuplicates(
    ["origin_airport_iata_code", "destination_airport_iata_code", "distance"],
)
...
df_cancellations = df_cancellations.dropDuplicates(
...
df_departures = df_departures.dropDuplicates(
...
df_arrivals = df_arrivals.dropDuplicates(
...
df_overall_schedules = df_overall_schedules.dropDuplicates(
    ["scheduled_time", "elapsed_time", "air_time"],
)

w = Window.orderBy("scheduled_time", "elapsed_time", "air_time")
df_overall_schedules = df_overall_schedules.withColumn(
    "schedules_id",
    row_number().over(w),
)

df_overall_schedules.write.mode("append").saveAsTable(
...
df_delays = df_delays.dropDuplicates(
    [
        "air_system_delay",
        "security_delay",
        "airline_delay",
